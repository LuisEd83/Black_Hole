...
static void sh_draw(unsigned int* h, unsigned int total) {
    ...
    double pct_done = total > 0 ? static_cast<double>(settled) / static_cast<double>(total) * 100.0 : 0.0;
    ...
    for(int i = 0; i < SH_NUM_STATES; i++){
        ...
        double frac  = (settled > 0) ? static_cast<double>(h[i]) / static_cast<double>(settled) : 0.0;
        int filled = static_cast<int>(frac * SH_BAR_W + 0.5);
...
static void* sh_poll_thread(void* arg){
    SH_State* sh = static_cast<SH_State*>(arg);
