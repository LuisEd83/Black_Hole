...
int main() {
    ...
    */
    ...
    glm::dvec3 world_up  = glm::dvec3(0.0, 0.0, 1.0);

    glm::dvec3 fwd   = glm::normalize(target - pos);
    glm::dvec3 right = glm::normalize(glm::cross(fwd, world_up));
    glm::dvec3 up    = glm::normalize(glm::cross(right, fwd));
    
    std::vector<unsigned char> pixels(static_cast<std::size_t>(WIDTH) * static_cast<std::size_t>(HEIGHT) * 3u, 0);
    ...
    std::cout << "    → distância câmera:    " << std::fixed << std::setprecision(1) << cam_dist * 10e-12 << "·10^12m" << " - (" << factor <<  "·RS)\n";
    ...
    */
    ...
    // double total_ms = 0.0;  // used when result section is enabled
    ...
    for (int i = 0; i < RUNS; i++) {
        ...
        StateHeatmap state_runs(static_cast<unsigned int>(WIDTH) * static_cast<unsigned int>(HEIGHT), BH::is_sim);
        ...
        // total_ms += ms;  // used when result section is enabled
    ...
    double total = static_cast<double>(WIDTH) * HEIGHT;
    ...
    */
    ...
    std::vector<unsigned char> png_pixels(static_cast<std::size_t>(WIDTH) * static_cast<std::size_t>(HEIGHT) * 4u);
    for (int i = 0; i < WIDTH * HEIGHT; i++) {
        std::size_t ii = static_cast<std::size_t>(i);
        png_pixels[ii*4+0] = pixels[ii*3+0];  // R
        png_pixels[ii*4+1] = pixels[ii*3+1];  // G
        png_pixels[ii*4+2] = pixels[ii*3+2];  // B
        png_pixels[ii*4+3] = 255;             // A
    }


    unsigned error = lodepng::encode("output/" + filename.str(), png_pixels,
