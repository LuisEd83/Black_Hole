...
static void sh_poll_fn(SH_State* sh, std::mutex* mtx){
...
class StateHeatmap {
    ...
    public:
...
#ifdef _WIN32
            // habilita códigos de escape ANSI no terminal do Windows 10+
        ...
        void start(unsigned int* d_counts){
            ...
            poll_thread_ = std::thread(sh_poll_fn, &sh_, &mutex_);
        }
