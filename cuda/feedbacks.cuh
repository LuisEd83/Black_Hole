...
inline void warmupAndEstimate(int WIDTH, int HEIGHT, int maxSteps, double step, double rs){
    ...
    // ── 1. Info do device ────────────────────────────────────────────────────
    std::cout << "       ◦ detectando device CUDA...\n";
    int deviceId;
    ...
    double estKernelMs = msPerPixel * WIDTH * HEIGHT;

    // memcpy: estimativa baseada na bandwidth teórica do device
    // (pegamos a bandwidth de memória e calculamos quanto tempo leva pra
    //  copiar o buffer do tamanho real de GPU → CPU)
    double bwGBs          = prop.memoryClockRate * 1e3       // Hz
                          * prop.memoryBusWidth / 8.0        // bytes/s
                          * 2.0                              // DDR
                          / 1e9;                             // GB/s
    double estMemcpyMs    = (realBufferSize / 1e9) / bwGBs * 1000.0;
    ...
    std::cout << "           ▶ kernel:" << std::setw(8) << std::fixed << std::setprecision(2) << estKernelMs   << " ms\n";
    ...
    if (estFps < 1.0) {
        std::cout << "  ⚠  Menos de 1 fps esperado. Considere:\n";
