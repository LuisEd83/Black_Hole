...
void warmupAndEstimate(int WIDTH, int HEIGHT, int maxSteps, double step, double rs){
    ...
    size_t realBufferSize = static_cast<size_t>(WIDTH) * static_cast<size_t>(HEIGHT) * 3u;
    ...
    std::chrono::duration<double, std::milli> allocDur = now() - t0;
    double allocMs = allocDur.count();
    ...
    std::chrono::duration<double, std::milli> driverInitDur = now() - t1;
    double driverInitMs = driverInitDur.count();
    ...
    for(int i = 0; i < BENCH_RUNS; i++){
        ...
        std::chrono::duration<double, std::milli> benchDur = now() - tb;
        totalBenchMs += benchDur.count();
    }
    ...
    std::chrono::duration<double, std::milli> memcpyDur = now() - tbw0;
    double estMemcpyMs = memcpyDur.count() * (static_cast<double>(realBufferSize) / static_cast<double>(testSize));
    ...
    int extra_seconds = static_cast<int>(estTotalMs / 1000.0);
