...
void launchPNG( unsigned char* pixels,
    ...
    size_t nbytes = static_cast<size_t>(rnd.WIDTH) * static_cast<size_t>(rnd.HEIGHT) * 3u;
    dim3 blockSize(16, 16);
    dim3 numBlocks(static_cast<unsigned>((rnd.WIDTH + 15)/16),
                   static_cast<unsigned>((rnd.HEIGHT + 15)/16));
    ...
    */
    ...
    if(ms_per_frame > 0.0){
        ...
        std::chrono::duration<double, std::milli> delta = now - t_last;
        // suaviza com média móvel exponencial — evita saltos por GC ou stall de driver
        ms_per_frame = ms_per_frame * 0.9 + delta.count() * 0.1;
    }
    t_last = Clock::now();
    //@}
    // ─────────────────────────────────────────────────────────────────────────────────────────────────
    ...
    cudaGetSymbolAddress(reinterpret_cast<void**>(&d_counts_ptr), d_state_counts);
