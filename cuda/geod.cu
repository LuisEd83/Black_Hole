...
__device__ inline double reconstructDphi(double L, double r, double sin_t) {
    double sin_t_safe = (fabs(sin_t) < 1e-6) ? 1e-6 : sin_t;
...
__device__  void rk4Step(Rays& s, double dl, double rs){
    ...
    //t.dphi = reconstructDphi(s.L, t.r, sin(t.theta));
    geodesicRHS(s, k1, rs);
